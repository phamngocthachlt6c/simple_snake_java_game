...
public class WindowGame extends JFrame implements KeyListener{
	...
	private int sizeOfSnakeBody = WINDOW_WIDTH / MATRIX_SIZE;
	private static final int DEFAULT_PERIOD = 400;
	
	private int headX = 6, headY = 4;
	private List bodyX = new ArrayList<Integer>();
	private List bodyY = new ArrayList<Integer>();
	private int direction = RIGHT;
	private Timer timer;
	
	private List foodPositionX = new ArrayList<Integer>();
	private List foodPositionY = new ArrayList<Integer>();
	...
	private boolean isSnakeHeadCollisionBody() {
		for(int i = 0; i < bodyX.size(); i++) {
			if((int) bodyX.get(i) == headX && (int) bodyY.get(i) == headY) {
				return true;
			}
		}
		return false;
	...
	private int[] getFoodPosition() {
		...
		result[0] = (int) foodPositionX.get(randomIndex);
		result[1] = (int) foodPositionY.get(randomIndex);
	...
	private int[] getNextPositionHead() {
		...
		switch(direction) {
			case UP: futureHeadY = headY-1; break;
			case DOWN: futureHeadY = headY+1; break;
			case LEFT: futureHeadX = headX-1; break;
			case RIGHT: futureHeadX = headX+1; break;
		}
		int[] result = new int[2];
	...
	private void moveSnake() {
		...
		if(futurePositionHead[0] < 0 || futurePositionHead[0] >= MATRIX_SIZE 
			...
			// Update snake body
			int oldTailX = (int) bodyX.get(bodyX.size() - 1);
			int oldTailY = (int) bodyY.get(bodyY.size() - 1);
...
//		super.paint(g);
		...
		for(int i = 0; i < bodyX.size(); i++) {
			// Draw the snake body
			g.fillRect((int) bodyX.get(i) * sizeOfSnakeBody + PADDING, (int) bodyY.get(i) * sizeOfSnakeBody + PADDING, sizeOfSnakeBody, sizeOfSnakeBody);
		}
